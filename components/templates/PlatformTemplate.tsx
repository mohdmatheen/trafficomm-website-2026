      ...
      <PageHero
        ...
        actions={
          <ButtonLink href="/contact" size="lg">
            Discuss {platform.name} Operations
          </ButtonLink>
        }
        aside={
          <div className="relative overflow-hidden rounded-[var(--radius-panel)] bg-ink p-7 text-white sm:p-9">
            ...
            <div className="relative">
              <div className="flex items-center gap-4">
                ...
                <span className="rounded-full bg-white px-4 py-2 text-[0.92rem] text-ink">{platform.name}</span>
              </div>
        ...
        <div className="grid gap-14 lg:grid-cols-2 lg:gap-20">
          ...
          <div>
            ...
            <ul className="mt-10 flex flex-wrap gap-2">
              {others.map((p) => (
                <li key={p.slug}>
                  <Link href={`/platforms/${p.slug}`} className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-[0.92rem] text-ink ring-1 ring-line transition-colors hover:ring-ink">
                    {p.name}
                  </Link>
                </li>
              ))}
